...
export const AdminDashboardPage: React.FC<AdminDashboardPageProps> = () => {
  ...
  const analytics = useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      if (o.orderMonth === '2026-09' || o.date.includes('Sep')) {
        sepRevenue += o.totalAmount;
      }
      if (o.currentStep < 4) {
        activePipelineValue += o.totalAmount;
        activeOrdersCount += 1;
      }
    });
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="p-6 sm:p-8 space-y-8 max-w-7xl">
          {(activeNav === 'dashboard' || activeNav === 'orders' || activeNav === 'analytics') && (
            ...
            {/* 3 Top Cards (Matching Screenshot Layout) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div className="bg-gradient-to-br from-[#FF5722] via-[#FF6A3D] to-[#E63946] rounded-3xl p-6 text-white shadow-xl shadow-[#FF5722]/20 relative overflow-hidden flex flex-col justify-between group">
                ...
                <div className="pt-4 border-t border-white/20 flex items-center justify-between text-xs font-medium">
                  <span className="text-white/90">See details</span>
                  <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>

              {/* Card 2: Dark Glass Card (Savings Account / September Sales) */}
              <div className="bg-[#14171E] border border-[#202530] rounded-3xl p-6 text-white shadow-xl flex flex-col justify-between hover:border-gray-700 transition-colors group">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-sm">September Sales</div>
                      <div className="text-[11px] text-gray-400">Current Month Inflow</div>
                    </div>
                  </div>
                ...
                <div className="my-2">
                  <div className="flex items-baseline gap-3">
                    <span className="text-3xl font-extrabold tracking-tight font-serif text-white">
                      {formatINR(analytics.sepRevenue)}
                    </span>
                ...
                <div className="my-2">
                  <div className="flex items-baseline gap-3">
                    ...
                    <span className="bg-[#FF5722]/10 text-[#FF5722] border border-[#FF5722]/30 text-[11px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
                      4 Orders <ArrowUpRight className="w-3 h-3" />
                    </span>
                  </div>
                </div>
          ...
          {/* ===================================================== */}
          {/* MIDDLE ROW: WALLET / CATEGORIES & CASH FLOW CHART     */}
          {/* ===================================================== */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            {/* Left Box: Collection Mediums (Matching "My Wallet") */}
            <div className="lg:col-span-5 bg-[#14171E] border border-[#202530] rounded-3xl p-6 shadow-xl flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <h3 className="font-bold text-sm text-white">Collection Portfolio</h3>
                  <button
                    onClick={openNewPaintingModal}
                    className="text-xs bg-[#FF5722] hover:bg-[#e64a19] text-white px-3 py-1 rounded-lg font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                  >
                ...
                <p className="text-xs text-gray-400 mb-5">
                  Fine art inventory status & valuation
                </p>

                {/* 4 Cards in 2x2 Grid (Matching Screenshot) */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-[#181C24] border border-[#242A36] rounded-2xl p-3.5">
                    ...
                    <div className="font-bold text-base text-white">₹6,80,000</div>
                    <div className="text-[10px] text-emerald-400 mt-1">8 Masterworks Active</div>
                  </div>
                  ...
                  <div className="bg-[#181C24] border border-[#242A36] rounded-2xl p-3.5">
                    ...
                    <div className="font-bold text-base text-white">₹4,25,000</div>
                    <div className="text-[10px] text-emerald-400 mt-1">3 Masterworks Active</div>
                  </div>
                  ...
                  <div className="bg-[#181C24] border border-[#242A36] rounded-2xl p-3.5">
                    ...
                    <div className="font-bold text-base text-white">₹1,90,000</div>
                    <div className="text-[10px] text-emerald-400 mt-1">5 Studies Active</div>
                  </div>
                  ...
                  <div className="bg-[#181C24] border border-[#242A36] rounded-2xl p-3.5">
                    ...
                    <div className="font-bold text-base text-white">₹85,000</div>
                    <div className="text-[10px] text-gray-400 mt-1">12 Prints Ready</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-gray-800 flex items-center justify-between text-xs text-gray-400">
                <span>Total Catalog Artworks</span>
                <span className="font-bold text-white">{artworks.length} Originals</span>
              </div>
            </div>

            {/* Right Box: Cash Flow Chart (Matching Screenshot) */}
            <div className="lg:col-span-7 bg-[#14171E] border border-[#202530] rounded-3xl p-6 shadow-xl">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                <div>
                  <div className="text-xs text-gray-400 font-medium">Cash Flow & Sales Trend</div>
                  <div className="text-2xl font-bold font-serif text-white mt-0.5">
                    {formatINR(analytics.totalRevenue)}
                  </div>
                </div>

                {/* Monthly / Yearly Toggle (Matching Screenshot) */}
              ...
              {/* Sleek Bar Chart (Matching Reference Screenshot) */}
              <div className="relative pt-6 pb-2">
                {/* Active Tooltip above the glowing bar */}
                <div className="hidden sm:flex absolute top-0 left-[68%] -translate-x-1/2 bg-[#1C202B] border border-[#2B3342] px-3 py-1.5 rounded-xl shadow-2xl flex-col text-[10px] text-gray-300 z-10">
                  <span className="text-gray-400">September 2026</span>
                  <span className="font-bold text-white text-xs">₹5,50,000</span>
                  <span className="text-emerald-400">+2 Masterworks Sold</span>
                </div>

                <div className="h-44 flex items-end justify-between gap-2 px-2 border-b border-gray-800 pb-2">
                  {/* Bar Jan */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-16 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Jan</span>
                  </div>

                  {/* Bar Feb */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-24 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Feb</span>
                  </div>

                  {/* Bar Mar */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-20 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Mar</span>
                  </div>

                  {/* Bar Apr */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-28 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Apr</span>
                  </div>

                  {/* Bar May */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-14 transition-all"></div>
                    <span className="text-[10px] text-gray-500">May</span>
                  </div>

                  {/* Bar Jun */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-22 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Jun</span>
                  </div>

                  {/* Bar Jul */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-32 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Jul</span>
                  </div>

                  {/* Bar Aug */}
                  <div className="flex-1 flex flex-col items-center gap-2 group">
                    <div className="w-full bg-[#1F2430] hover:bg-[#2A3142] rounded-xl h-36 transition-all"></div>
                    <span className="text-[10px] text-gray-500">Aug</span>
                  </div>

                  {/* Bar Sep (Active Glowing Orange/White Gradient Bar - Exactly like screenshot) */}
                  <div className="flex-1 flex flex-col items-center gap-2 relative">
                    <div className="w-full bg-gradient-to-t from-[#FF5722] via-[#FF7A45] to-white rounded-xl h-40 shadow-lg shadow-[#FF5722]/30 ring-2 ring-[#FF5722]/50 relative">
                      <div className="absolute top-2 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-white animate-pulse"></div>
                    </div>
                    <span className="text-[10px] font-bold text-[#FF5722]">Sep</span>
                  </div>
                </div>
              </div>
            </div>
